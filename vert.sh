...
build_lua () {
  ...
  if [[ ! "$1" ]]
  then
    echo "build_lua takes 3 arguments given: 0"
  fi
  local version=$1
  
  if [[ ! "$2" ]]
  then
    echo "build_lua takes 3 arguments given: 1"
  fi
  local platform=$2

  if [[ ! "$3" ]]
  then
    echo "build_lua takes 3 arguments given: 2"
  fi
  local target_dir=$3
...
build_luarocks () {
  ...
  if [[ ! "$1" ]]
  then
    echo "build_luarocks takes 4 arguments given: 0"
  fi
  local version=$1
  ...
  if [[ ! "$2" ]]
  then
    echo "build_luarocks takes 4 arguments given: 1"
  fi
  local prefix=$2

  if [[ ! "$3" ]]
  then
    echo "build_luarocks takes 4 arguments given: 2"
  fi
  local sysconfdir=$3

  if [[ ! "$4" ]]
  then
    echo "build_luarocks takes 4 arguments given: 3"
  fi
  local with_lua=$4
