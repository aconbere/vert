...
function _M.do_init(opts)
  ...
  local luarocks_version   = opts["luarocks_version"]
  local lua_version        = opts["lua_version"]
  ...
  local installer = installers[lua_implimentation]

  installer:install({ build_dir = build_dir
                    , version   = lua_version
                    , platform  = platform
                    , prefix    = prefix
                    })
  ...
  activate.write(activate_template,
                 (installer._version or lua_version):sub(1,3),
                 prefix)
